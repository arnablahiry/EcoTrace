...
Return JSON ONLY with keys: name, brand, categories, packaging, labels, ingredients, ecoscore, nutriscore, imageUrl.`;
  ...
  if (!imageBase64 && !isValidImageUrl(productDetails.imageUrl)) {
  ...
  if (
    ...
    !isValidImageUrl(productDetails.imageUrl)
  ) {
    await hydrateImageFromOff(productDetails.name);
  }
  ...
  const debugLine = offImageDebug ? `\n\nOFF image debug: ${offImageDebug}` : "";
  return {
