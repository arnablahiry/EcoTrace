...
function GreenScannerWidget() {
  ...
  const debugImageUrl = useMemo(() => {
    if (!responseText.includes("OFF top image debug:")) return "";
    const match = responseText.match(/OFF top image debug:\s*(https?:\/\/\S+)/);
  ...
  const buildImageSrc = (url?: string | null) => {
    ...
    const cacheBuster = product?.name
      ? `v=${encodeURIComponent(product.name)}`
      : `v=${Date.now()}`;
    if (url.includes("images.openfoodfacts.org")) {
      const separator = url.includes("?") ? "&" : "?";
      return `${url}${separator}${cacheBuster}`;
    }
    return `/api/image-proxy?url=${encodeURIComponent(url)}&${cacheBuster}`;
  };
  ...
  const resolvedImageUrl = isImageMode
    ? fallbackImage || placeholderImage
    : (isValidImageUrl(debugImageUrl) ? buildImageSrc(debugImageUrl) : "") ||
  ...
  return (
    <div className="page">
      <div className="shell">
        ...
        <section className={`card ${structured ? "has-results" : ""}`}>
          <form
            className="form"
            onSubmit={(event) => {
              event.preventDefault();
          ...
          >
            ...
            >
              ...
              <label
                ...
                aria-disabled={!!structured}
                tabIndex={structured ? -1 : 0}
              >
              ...
              <button
                type="submit"
                disabled={!!structured || isPending || (!query.trim() && !imageReady)}
              >
