...
export async function buildSustainableAlternativeResult(product_query, imageBase64) {
    ...
    const isScoreGood = (score) => score.toUpperCase() === "A";
    const goodChoiceMessage = (eco, nutri) => {
    ...
    const fetchBraveResults = async (query) => {
    ...
    const normalizeField = (value, fallback = "Estimated") => {
    ...
    const parseStructuredOutput = (data) => {
        const candidates = [];
    ...
    const estimateWithOpenAI = async (name, webResults, imageData) => {
...
Return JSON ONLY with keys: name, brand, categories, packaging, labels, ingredients, ecoscore, nutriscore, imageUrl.`;
        ...
        const res = await withTimeout(fetch("https://api.openai.com/v1/responses", {
            method: "POST",
            headers: {
    ...
    const identifyProductFromImage = async (imageData) => {
        ...
        if (!apiKey)
            return null;
        const system = "Identify the packaged grocery product in the image. Return JSON ONLY with keys: name, brand. " +
    ...
    let imageDerivedName = null;
    let imageDerivedBrand = null;
    ...
    const pickAlternatives = (altProducts, ecoScore, nutri, originalName) => {
        ...
        const scored = altProducts
            ...
            const scoreRaw = typeof p.ecoscore_grade === "string"
                ? p.ecoscore_grade.toUpperCase()
                : "?";
            const nutriRaw = typeof p.nutriscore_grade === "string"
            ...
            return {
                ...
                ingredients: typeof p.ingredients_text === "string" && p.ingredients_text.trim()
                    ...
                    : "Unknown",
                nutriScore: nutriRaw,
                imageUrl: (typeof p.image_front_url === "string" && p.image_front_url) ||
                    (typeof p.image_url === "string" && p.image_url) ||
                    "",
            };
        })
            .filter((p) => p.name && p.name.toLowerCase() !== originalName.toLowerCase())
            .filter((p) => scoreRank(p.score) <= ecoRank && scoreRank(p.nutriScore) <= nutriRank);
        const buckets = {
            A: [],
            B: [],
            C: [],
    ...
    const isUnknownScore = (value) => {
        const normalized = (value || "").trim().toUpperCase();
        return !normalized || normalized === "?" || normalized === "UNKNOWN";
    };
    const isValidImageUrl = (value) => typeof value === "string" &&
        (value.startsWith("http://") ||
            value.startsWith("https://") ||
            value.startsWith("data:image"));
    const pickImageFromOffProducts = (products, query, brand) => {
        ...
        const scoreProduct = (p) => {
            ...
            if (name && normalizedQuery && name.includes(normalizedQuery))
                score += 4;
            if (normalizedQuery && name && normalizedQuery.includes(name))
                score += 3;
            ...
            for (const token of brandTokens) {
                if (token && brands.includes(token))
                    score += 2;
            }
            return score;
        };
        const withImage = products.filter((p) => typeof p.image_front_url === "string" || typeof p.image_url === "string");
        if (withImage.length === 0)
            return "";
        const best = withImage.sort((a, b) => scoreProduct(b) - scoreProduct(a))[0];
        return ((typeof best?.image_front_url === "string" && best.image_front_url) ||
            (typeof best?.image_url === "string" && best.image_url) ||
            "");
    };
    const isMissingDetails = (details) => details.brand === "Unknown" ||
    ...
    const searchUrl = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(effectiveQuery)}&search_simple=1&action=process&json=1`;
    ...
    if (!searchData.products || searchData.products.length === 0) {
        const fallbackUrl = `https://world.openfoodfacts.org/api/v2/search?search_terms=${encodeURIComponent(effectiveQuery)}&sort_by=popularity&page_size=24&fields=product_name,brands,ecoscore_grade,categories_tags,packaging_tags,labels_tags,nutriscore_grade,ingredients_text,image_url,image_front_url`;
    ...
    const nutriScore = typeof product.nutriscore_grade === "string"
    ...
    const imageUrl = (typeof product.image_front_url === "string" && product.image_front_url) ||
        (typeof product.image_url === "string" && product.image_url) ||
        "";
    const ingredients = typeof product.ingredients_text === "string" && product.ingredients_text.trim()
    ...
    if (needsEstimation) {
        ...
        if (estimate) {
            ...
            productDetails.ingredients = normalizeField(estimate.ingredients, productDetails.ingredients);
    ...
    let offImageDebug = "";
    const fetchTopOffImage = async (query) => {
        const imageSearchUrl = `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(query)}&search_simple=1&action=process&json=1&page_size=1`;
        ...
        return ((typeof first.image_front_url === "string" && first.image_front_url) ||
            (typeof first.image_url === "string" && first.image_url) ||
            "");
    };
    const hydrateImageFromOff = async (query) => {
        const searchQueries = [];
        ...
        for (const q of searchQueries) {
            const imageSearchUrl = `https://world.openfoodfacts.org/api/v2/search?search_terms=${encodeURIComponent(q)}&page_size=12&fields=product_name,brands,image_url,image_front_url`;
            const imageSearchRes = await fetch(imageSearchUrl);
            if (imageSearchRes.ok) {
                const imageSearchData = await imageSearchRes.json();
                const imageProducts = Array.isArray(imageSearchData.products)
                    ? imageSearchData.products
                    : [];
                const imageUrl = pickImageFromOffProducts(imageProducts, q, productDetails.brand);
                if (!offImageDebug) {
    ...
    const hydrateTopImageFromOff = async (query) => {
        const imageSearchUrl = `https://world.openfoodfacts.org/api/v2/search?search_terms=${encodeURIComponent(query)}&page_size=1&fields=product_name,brands,image_url,image_front_url`;
        const imageSearchRes = await fetch(imageSearchUrl);
        if (!imageSearchRes.ok)
            return false;
        const imageSearchData = await imageSearchRes.json();
        const imageProducts = Array.isArray(imageSearchData.products)
            ...
            : [];
        const first = imageProducts[0] || {};
        const imageUrl = (typeof first.image_front_url === "string" && first.image_front_url) ||
            (typeof first.image_url === "string" && first.image_url) ||
            "";
        if (isValidImageUrl(imageUrl)) {
            productDetails.imageUrl = imageUrl;
    ...
    if (!imageBase64) {
        if (productDetails.ecoEstimated ||
    ...
    const productMessages = goodChoiceMessage(productDetails.ecoscore, productDetails.nutriscore);
    const buildAltUrl = (useCategory) => {
        const base = "https://world.openfoodfacts.org/api/v2/search";
        const params = new URLSearchParams({
            sort_by: "popularity",
            page_size: "24",
            fields: "product_name,brands,ecoscore_grade,categories_tags,packaging_tags,labels_tags,nutriscore_grade,ingredients_text,image_url,image_front_url",
        });
    ...
    let alternatives = pickAlternatives(altProducts, productDetails.ecoscore, productDetails.nutriscore, productDetails.name);
    if (alternatives.length < 3) {
        ...
        if (fallbackRes.ok) {
            ...
            const merged = [
                ...alternatives,
                ...pickAlternatives(fallbackProducts, productDetails.ecoscore, productDetails.nutriscore, productDetails.name),
            ];
