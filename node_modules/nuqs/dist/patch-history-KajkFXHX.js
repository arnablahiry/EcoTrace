...
import { r as spinQueueResetMutex, t as resetQueues } from "./reset-t0vXtVbG.js";
...
function shouldPatchHistory(adapter) {
	...
	if (history.nuqs?.version && history.nuqs.version !== "2.8.8") {
		console.error(error(409), history.nuqs.version, `2.8.8`, adapter);
		return false;
	}
...
function patchHistory(emitter, adapter) {
	...
	debug("[nuqs %s] Patching history (%s adapter)", "2.8.8", adapter);
	function sync(url) {
		spinQueueResetMutex();
		try {
